...
client.on("ready", () => {
  ...
  client.user.setPresence({ activity: { name: "Les infos sur le covid", type: 'WATCHING' }, status: 'idle' });
})

// définition de la date actuelle
const datetime = new Date()
let datefinal = datetime.getDate() + "-" + (datetime.getMonth() + 1) + "-" + datetime.getFullYear() + "-" + datetime.getHours() + "H" + datetime.getMinutes()

// Ce bout de code ci dessous peut être ajouter au reste pour avoir les minutes et secondes en plus
//  + datetime.getMinutes() + ":" + datetime.getSeconds()

cron.schedule('50 12,23 * * *', async() => {

      const downloader = new Downloader({
        url: 'https://disease.sh/v3/covid-19/countries/France?yesterday=true&twoDaysAgo=true&strict=true&allowNull=true', //url du fichier, pour le personaliser -> https://disease.sh/docs/#/
      ...
      const attachment = new MessageAttachment(buffer, `${datefinal}.json`);
